...
def api_error(code: str, message: str, status_code: int, details: dict | None = None) -> HTTPException:
    return HTTPException(status_code, detail={"error": {"code": code, "message": message, "details": details or {}}})
...
async def upload_document(
    ...
    existing = db.scalar(select(TranslationJob).where(
    ...
    if len(content) > settings.max_file_size_bytes:
        raise api_error("PLAN_LIMIT_EXCEEDED", "El archivo supera el limite permitido.", 413,
                        {"metric_code": "max_file_size_bytes", "limit_value": settings.max_file_size_bytes,
    ...
    document = Document(id=document_id, owner_key=owner_key, original_filename=file.filename,
                        mime_type="application/pdf", size_bytes=len(content), content_hash=content_hash,
                        storage_path=storage_path)
    job = TranslationJob(document_id=document_id, owner_key=owner_key, idempotency_key=idempotency_key,
...
def _response(document: Document, job: TranslationJob) -> dict[str, object]:
    return {"document": {"document_id": document.id, "original_filename": document.original_filename,
                          "mime_type": document.mime_type, "size_bytes": document.size_bytes,
                          "status": document.status},
            "job": {"job_id": job.id, "status": job.status, "progress_percent": job.progress_percent,
                     "current_step": job.current_step, "requested_at": job.requested_at}}
